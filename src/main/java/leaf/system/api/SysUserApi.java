...
//                是否允许后台登录 (在未禁用的角色中至少有一个允许后台登录，返回1，否则返回0)
        ...
        if("1".equals(isBackend)) {
            if("1".equals(isSaveLoginStatus)) {
                try {
                    time = Integer.parseInt(Config.Properties.getProperty("system.login.backend_time","2592000"));
            ...
            if("1".equals(isSaveLoginStatus)) {
                try {
                    time = Integer.parseInt(Config.Properties.getProperty("system.login.frontend_time","7776000"));
                } catch(Exception ignored) {}
            }
    ...
    public JSONMap isLogin() {
        ...
        JSONList userAll = DB.query("" +
                "select a.name,a.account,a.personal_signature,a.avatar,a.phone,a.email,a.sex,a.is_disable,c.role_id,c.role_name " +
                "from sys_user a " +
                "   left join sys_user_role_rel b on a.user_id = b.user_id " +
                "   left join sys_role c on b.role_id = c.role_id " +
...
//    }
     ...
     */
    @GetMapping("/system/api/user/getUserList")
    @LoginToken(validBackend = true,permissionKey = "lspk:system:user:list")
    public JSONMap getUserList() {
        ...
        String masterListSql = "" +
                "select aa.user_id,aa.name,aa.account,aa.password,aa.personal_signature,aa.avatar,aa.birthday,aa.phone,aa.email," +
        ...
        String sql = "" +
                "select a.user_id,name,account,password,personal_signature,avatar,birthday,phone,email,real_name,idcard," +
        ...
        for(int i = 0;i < userList.size();i++) {
            ...
            if(user == null) {
                ...
                user.put("password",userList.getMap(i).get("password"));
    ...
    public JSONMap updateUser() {
        ...
        switch(updateType) {
            case "Edit":
                if(!ApiGlobalInterceptor.permission("lspk:system:user:edit")) {
                ...
                String sql = "" +
                        "update sys_user " +
                        "set name = '"+DB.e(name)+"',account = '"+DB.e(account)+"',password = '"+DB.e(password)+"',personal_signature = '"+DB.e(personalSignature)+"'," +
                ...
                break;
            case "Add":
                if(!ApiGlobalInterceptor.permission("lspk:system:user:add")) {
                    Http.write(403,JSONMap.error("接口执行失败，该用户没有权限"));
                    return null;
                }
                ...
                break;
            case "Delete":
                if(!ApiGlobalInterceptor.permission("lspk:system:user:delete")) {
                    Http.write(403,JSONMap.error("接口执行失败，该用户没有权限"));
                    return null;
                }
                ...
                break;
            case "IsDisable":
                if(!ApiGlobalInterceptor.permission("lspk:system:user:edit:isDisable")) {
                    Http.write(403,JSONMap.error("接口执行失败，该用户没有权限"));
                    return null;
                }
    ...
    public JSONMap batchUpdateUser() {
        ...
        switch(updateType) {
            case "IsDisable":
                ...
                break;
//            case "Sex":
//                if(!ApiGlobalInterceptor.permission("lspk:system:user:edit:sex")) {
//                    Http.write(403,JSONMap.error("接口执行失败，该用户没有权限"));
//                    return null;
//                }
//
