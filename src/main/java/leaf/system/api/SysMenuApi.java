...
//        System.out.println(Log.content("DEBUG","获取菜单tree没有走缓存"));
        ...
        if("1".equals(userRoleIdStr) || userRoleIdStr.startsWith("1,")) {
            sql = "" +
                    "select menu_id,menu_name,parent_menu_id,menu_icon,url,type " +
                    "from sys_menu " +
                    "where is_show = 1 and type in (1,2,3,4,5) " +
                    "order by sort desc";
        } else {
            sql = "" +
                    "select distinct a.menu_id,a.menu_name,a.parent_menu_id,a.menu_icon,a.url,a.type " +
                    "from sys_menu a " +
                    "   left join sys_role_menu_rel b on a.menu_id = b.menu_id " +
                    "where is_show = 1 and type > 0 and b.role_id in (" + DB.e(userRoleIdStr) + ") " +
                    "order by sort desc";
        }
    ...
    @LoginToken(validBackend = true,permissionKey = "PermissionKey-system_menu_getMenuList")
    public JSONMap getMenuList() {
        ...
        String sql = "" +
                "select a.menu_id,a.menu_name,a.parent_menu_id,b.menu_name parent_menu_name,a.menu_icon,a.url,a.type,a.permission_key,a.is_show,a.sort " +
                "from sys_menu a " +
...
//    @CacheEvict(cacheNames = "SpringCache_system_menu",key = "'MenuTree'")//修改菜单后会删除getMenuTree接口返回值的缓存
    public JSONMap updateMenu() {
        ...
        switch(updateType) {
            case "Edit":
                if(!ApiGlobalInterceptor.permission("PermissionKey-system_menu_updateMenuEdit")) {
                ...
                String sql = "" +
                        ...
                        "   menu_icon = '"+DB.e(menuIcon)+"',url = '"+DB.e(url)+"',type = '"+type+"',permission_key = '"+DB.e(permissionKey)+"',is_show = '"+isShow+"' " +
                ...
                break;
            case "Add":
                if(!ApiGlobalInterceptor.permission("PermissionKey-system_menu_updateMenuAdd")) {
                    Http.write(403,JSONMap.error("接口执行失败，该用户没有权限"));
                    return null;
                }
                ...
                sql = "" +
                        "insert into sys_menu(menu_name,parent_menu_id,menu_icon,url,type,permission_key,is_show)" +
                        "value('"+DB.e(menuName)+"',"+(parentMenuId.isEmpty() || !Valid.isInteger(parentMenuId)?"null":""+parentMenuId+"")+"," +
                        "   '"+DB.e(menuIcon)+"','"+DB.e(url)+"','"+type+"','"+DB.e(permissionKey)+"','"+isShow+"');" +
...
//                if(DB.updateTransaction(sql) > 0) return JSONMap.success();
                break;
            case "Delete":
                if(!ApiGlobalInterceptor.permission("PermissionKey-system_menu_updateMenuDelete")) {
                    Http.write(403,JSONMap.error("接口执行失败，该用户没有权限"));
                    return null;
                }
                ...
                break;
            case "Sort":
                if(!ApiGlobalInterceptor.permission("PermissionKey-system_menu_updateMenuSort")) {
                    Http.write(403,JSONMap.error("接口执行失败，该用户没有权限"));
                    return null;
                }
            ...
            case "IsShow":
                if(!ApiGlobalInterceptor.permission("PermissionKey-system_menu_updateMenuIsShow")) {
                    Http.write(403,JSONMap.error("接口执行失败，该用户没有权限"));
                    return null;
                }
                ...
                if(DB.update("update sys_menu set is_show = "+isShow+" where menu_id = '"+DB.e(menuId)+"'") > 0) {
                    return JSONMap.success();
                }
                break;
            default:
