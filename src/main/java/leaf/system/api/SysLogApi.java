...
public class SysLogApi {
    ...
    @LoginToken(validBackend = true,permissionKey = "lyfsysPermissionKey-system_log_getLog")
                        ...
                        for(int k = dayDirs.length - 1;k >= 0;k--) {
                            ...
                            if(dayDirs[k].isDirectory()) {
                                ...
                                for(File logFile:logFiles) {
                                    ...
                                    log = new JSONMap();
                                    log.put("type","file");
                                    log.put("name",Str.removeSuffix(logName,".log"));
    ...
    public JSONMap updateLog() {
        ...
        switch(updateType) {
            case "Rename":
                if(!ApiGlobalInterceptor.permission("lyfsysPermissionKey-system_log_updateLogRename")) {
                ...
                break;
            case "Delete":
                if(!ApiGlobalInterceptor.permission("lyfsysPermissionKey-system_log_updateLogDelete")) {
                    Http.write(403,JSONMap.error("接口执行失败，该用户没有权限"));
                    return null;
                }
