...
public class SysCommon {
    ...
    public static String getResourcePath() {
        return System.getProperty("user.dir") + Config.Properties.getProperty("system.file.path","\\resources\\");
    }
     ...
     * @return 配置值
     */
    public static JSONMap getSystemConfig(String ... keys) {
     ...
     * @return 1:验证成功 2:验证失败 3:验证超时
     */
    public static int checkValidPuzzle(String validParam,int x) {
        validParam = Lock.aesDncrypt(validParam);
        ...
        try {
            range = Integer.parseInt(""+ Config.Properties.get("system.validPuzzle.range"));
        } catch(Exception e) {}

        if(Math.abs(x - _x) <= range) {
            Cache.remove(validParam);
            return 1;
        } else {
            return 2;
