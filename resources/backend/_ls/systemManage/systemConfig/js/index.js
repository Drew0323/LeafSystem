function init(props, params) {
    ...
    new Vue({
        ...
        data: {
            ...
            // 表格字段
            tableColumns: [{
                title: '操作', width: 120, fixed: "left", scopedSlots: { customRender: 'action' }
            }, , {
                title: 'ID', dataIndex: 'config_id', align: 'center', sorter: true, width: 100, fixed: "left"
            }, {
                title: '配置键', dataIndex: 'config_key', align: 'center', width: 200, sorter: true
            }, {
                title: '配置值', dataIndex: 'config_value', align: 'center', minWidth: 250, scopedSlots: { customRender: 'config_value' }
            }, {
                title: '配置描述', dataIndex: 'config_desc', align: 'center', minWidth: 250, scopedSlots: { customRender: 'config_desc' }
            }, {
                title: '配置类型', dataIndex: 'config_type', align: 'center', sorter: true, width: 100, scopedSlots: { customRender: 'config_type' }
            }],
            // 表格数据
            tableData: [],
            tableLoading: false,
        ...
        methods: {
            ...
            edit(id,operate) {
                if (operate === 'Delete') {
                    Ajax.post({
                        url: '/system/api/systemConfig/updateSystemConfig?UpdateType=Delete',
                        param: {
                            config_id: id
                        },
                        success(result) {
            ...
            onSelectChange(selectedKeys,selectedItems) {
                ...
                this.selectedIds = selectedItems.map(item => item.config_id);
            },
            // 初始化Viewer
            initViewer() {
                // 如果 viewer 已存在，销毁旧实例
